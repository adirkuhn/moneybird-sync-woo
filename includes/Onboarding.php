...
class Onboarding
{
	...
	{
		// Register the page callback so WordPress actually calls render() when
		// the user visits admin.php?page=mb-onboarding. AdminUI adds the visible
		// submenu link, but uses '__return_null' as its callback — meaning the
		// page content would otherwise be empty.
		add_submenu_page(
			null,                              // hidden from menu (AdminUI owns the visible link)
			__('Moneybird Sync — Setup', 'moneybird-sync-for-woo'),
			__('Setup', 'moneybird-sync-for-woo'),
			'manage_woocommerce',
			self::PAGE_SLUG,
			array($this, 'render')
		);
	...
	public function handle_requests(): void
	{
		...
		if (isset($_POST['onboarding_oauth_settings'])) {
			$this->handle_step1_save();
		}

		$action = isset($_GET['mbsfw_oauth']) ? 'oauth_' . sanitize_key((string) $_GET['mbsfw_oauth']) : '';

		if ('oauth_callback' === $action || (isset($_GET['code']) && isset($_GET['state']))) {
	...
	private function handle_step1_save(): void
	{
		$client_id = isset($_POST['client_id']) ? sanitize_text_field((string) $_POST['client_id']) : '';
		$client_secret = isset($_POST['client_secret']) ? sanitize_text_field((string) $_POST['client_secret']) : '';
		if (!$client_id || !$client_secret) {
			wp_safe_redirect(admin_url('admin.php?page=' . self::PAGE_SLUG . '&step=1&error=no_credentials'));
			exit;
		}
	...
	private function render_step1(): void
	{
		...
		<p><?php esc_html_e('Click the button below to authorize this plugin to access your Moneybird account.', 'moneybird-sync-for-woo'); ?>
		</p>

		<?php if (!$this->oauth_credentials_configured()): ?>
			<!-- add form to save the clientid and secret -->
			<form method="post" name="onboarding_oauth_settings">
				<?php wp_nonce_field('mbsfw_onboarding'); ?>
				<input type="hidden" name="onboarding_oauth_settings" value="1" />
				<input type="hidden" name="mbsfw_step" value="1" />
				<table class="form-table">
					<tr>
						<th scope="row"><label for="client_id">
								<?php esc_html_e('Client ID', 'moneybird-sync-for-woo'); ?>
							</label></th>
						<td>
							<input name="client_id" type="text" id="client_id" value="" class="regular-text" required />
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="client_secret">
								<?php esc_html_e('Client Secret', 'moneybird-sync-for-woo'); ?>
							</label></th>
						<td>
							<input name="client_secret" type="text" id="client_secret" value="" class="regular-text" required />
						</td>
					</tr>
				</table>
				<?php submit_button(__('Save Credentials', 'moneybird-sync-for-woo')); ?>
			</form>
		<?php else: ?>
	...
	private function render_step2(): void
	{
		...
		<form method="post">
			...
			<table class="form-table">
				<tr>
					<th scope="row"><label
							for="administration_id"><?php esc_html_e('Administration', 'moneybird-sync-for-woo'); ?></label>
					</th>
					<td>
						<select name="administration_id" id="administration_id" required>
							<option value=""><?php esc_html_e('— Select an administration —', 'moneybird-sync-for-woo'); ?>
							</option>
							<?php foreach ($administrations as $admin): ?>
								<option value="<?php echo esc_attr((string) $admin['id']); ?>" <?php selected($current, (string) $admin['id']); ?>>
			...
			<p class="submit">
				<button type="submit"
					class="button button-primary"><?php esc_html_e('Save & Continue', 'moneybird-sync-for-woo'); ?></button>
			</p>
		</form>
		<?php
	}
	...
	private function render_step3(): void
	{
		...
		$settings = AdminUI::get_settings();
		?>
		<p><?php esc_html_e('Map your Moneybird financial accounts to their roles in the Stripe clearing model.', 'moneybird-sync-for-woo'); ?>
		</p>
		<form method="post">
			<?php wp_nonce_field('mbsfw_onboarding'); ?>
			...
			<table class="form-table">
				<tr>
					<th scope="row"><label
							for="clearing_account_id"><?php esc_html_e('Stripe Clearing Account', 'moneybird-sync-for-woo'); ?>
					...
					<td>
						...
						<p class="description">
							<?php esc_html_e('All Stripe payments land here. Never the bank.', 'moneybird-sync-for-woo'); ?>
						</p>
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="bank_account_id"><?php esc_html_e('Bank Account', 'moneybird-sync-for-woo'); ?>
							<span class="required">*</span></label></th>
					<td>
						<?php $this->render_account_select('bank_account_id', $financial_accounts, (string) ($settings['bank_account_id'] ?? '')); ?>
						<p class="description">
							<?php esc_html_e('Your actual bank. Stripe payouts are transferred here.', 'moneybird-sync-for-woo'); ?>
						</p>
					</td>
				</tr>
				<tr>
					<th scope="row"><label
							for="fees_ledger_account_id"><?php esc_html_e('Stripe Fees Ledger Account', 'moneybird-sync-for-woo'); ?></label>
					</th>
					<td>
						<?php $this->render_ledger_select('fees_ledger_account_id', $ledger_accounts, (string) ($settings['fees_ledger_account_id'] ?? '')); ?>
						<p class="description">
							<?php esc_html_e('Expense account debited for Stripe transaction fees.', 'moneybird-sync-for-woo'); ?>
						</p>
					</td>
				</tr>
			</table>
	...
	private function render_step4(): void
	{
		...
		?>
		<p><?php esc_html_e('Click the button to verify your credentials and selected accounts are valid.', 'moneybird-sync-for-woo'); ?>
		</p>
		<form method="post">
			<?php wp_nonce_field('mbsfw_onboarding'); ?>
			...
			<p class="submit">
				<button type="submit"
					class="button button-primary"><?php esc_html_e('Test Connection', 'moneybird-sync-for-woo'); ?></button>
			</p>
		</form>
		<?php
	}
	...
	private function render_step5(): void
	{
		...
		?>
		<p><?php esc_html_e('Everything looks good! Enable the sync to start processing new WooCommerce Stripe orders into Moneybird.', 'moneybird-sync-for-woo'); ?>
		</p>
		...
		<?php if ($enabled): ?>
			<div class="notice notice-success inline">
				<p><?php esc_html_e('✓ Sync is already enabled. New orders will be queued automatically.', 'moneybird-sync-for-woo'); ?>
				</p>
			</div>
		...
		<?php else: ?>
			<form method="post">
				...
				<p class="submit">
					<button type="submit"
						class="button button-primary button-hero"><?php esc_html_e('✓ Enable Sync', 'moneybird-sync-for-woo'); ?></button>
				</p>
			</form>
		<?php endif; ?>
	<?php
